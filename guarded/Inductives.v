...
(** List of known defects:
  ...
  - effectively impossible to debug due to the a) slow-as-hell and lazy reduction employed by MetaCoq Run
                                               b) string stuff is super slow in Coq 

    When working with the trace monad, it is so slow in fact that its observational behaviour is equivalent to divergence on anything interesting :)
    This is because the reduction employed by MetaCoq Run is a) lazy b) implements laziness in a seemingly very broken way where terms are copied around all the time (instead of using sharing).
    Together with the fact that the checker needs to reduce all the time, the applications of the reduction function whd_all just get carried around with the HUGE representation of 
    the environments Σ, Γ. 
    And somehow, in most cases when the result is needed, it then takes forever to evaluate (even though there is a reduction path taking like two steps to reach a normal form by just unfolding the head constant... 

    This leads to huge explosions in a) runtime and b) memory usage (it's very easy to fill 20GB of RAM with quite trivial examples thanks to the borked reduction strategy and the complete lack of structure sharing in MC's reduction).
